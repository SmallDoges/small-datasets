...
from small_thoughts.utils.deduplicate import fuzz_string_pair
from small_thoughts.utils.eval import EVALUATION_DATASETS
...
def decontaminate(
    ...
    for eval_name, eval_info in evals.items():
        ...
        if eval_subset is not None:
            ds = load_dataset(eval_name, eval_subset, split=eval_splits, trust_remote_code=True, cache_dir=cache_dir, num_proc=num_proc)
        else:
            ds = load_dataset(eval_name, split=eval_splits, trust_remote_code=True, cache_dir=cache_dir, num_proc=num_proc)
