"""Rejection sampling for code. Execution logic heavily adapted from https://github.com/NovaSky-AI/SkyThought/tree/main/skythought/tools/util/taco"""
...
def run_test(sample, test=None, debug=False):
    ...
    try:
        in_outs = json.loads(sample["test_cases"])
    except ValueError:
        in_outs = None
...
def check_correct(problem, generation):
    ...
    try:
        ...
        print(f"Error in check_correct: {e}")
        return False
...
def process_single_row(row: dict) -> dict:
    ...
    Returns:
        dict: Processed row with correct evaluation
    """
    try:
        ...
        if check_correct(row, last_code):
            row["correct"] = True
            row["reason"] = ""
        else:
            row["correct"] = False
        ...
        return {**row, "correct": False, "reason": f"Processing error: {str(e)}"}


def code_judge(df: Dataset, num_cpus: int = None, batch_size: int = 2048) -> Dataset:
    """Judge a code dataset.
    ...
    Returns:
        Dataset: Processed dataset with correct evaluations
    """
    if num_cpus is None:
