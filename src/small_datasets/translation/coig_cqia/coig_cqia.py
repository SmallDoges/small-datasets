...
def extract_dataset_by_role(dataset):
    ...
    for i, sample in enumerate(dataset):
        ...
        if instruction != "" and input_text != "":
            # 两者都非空：instruction作为system，input作为user
            system_messages.append(instruction)
            system_sample_indices.append(i)
            
            user_messages.append(input_text)
            user_sample_indices.append(i)
        elif instruction != "":
            # 仅instruction非空，作为user
    ...
    }) if len(user_messages) > 0 else Dataset.from_dict({
        "content": [],
        "sample_indices": [],
    })
    ...
    }) if len(assistant_messages) > 0 else Dataset.from_dict({
        "content": [],
        "sample_indices": [],
    })
    ...
    }) if len(system_messages) > 0 else Dataset.from_dict({
        "content": [],
        "sample_indices": [],
    })
